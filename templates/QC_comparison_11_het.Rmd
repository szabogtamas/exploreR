...
for (PARAM in param_order) {
  ...
  pr_data <- main_data %>%
    ...
    map(function(x){
      ...
      if (nrow(x2) > 3){
        st <- wilcox.test(x2$Value, x2$Base_mean, paired = TRUE, alternative = "two.sided")
...
datatable(paired_wx_res)
```

## Parametric test after lognormalization
