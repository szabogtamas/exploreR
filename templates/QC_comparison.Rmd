...
mean_measured_values <- main_data %>%
  group_by(Param) %>%
  mutate(Base_mean = mean(Value, na.rm = TRUE)) %>%
  ungroup() %>%
  distinct(Param, Base_mean)
...
pct_diff_values <- main_data %>%
  left_join(mean_measured_values, by = "Param") %>%
  mutate(
...
### Difference by Machines

```{r message = FALSE, warning = FALSE}
# Interactively show pct_differences

for (MCH in unique(pct_diff_values$Mach)) {
  gg <- pct_diff_values %>%
    filter(Mach == MCH) %>%
    mutate(Param = paste(Param, "[", Unit, "]")) %>%
    ggplot(
      aes(x = Param, y = pct_diff, color = Sample, label1 = Value, label2 = Base_mean)
    ) +
  ...
  p <- gg %>%
    ggplotly(
      tooltip = c("x", "y", "label1", "label2"), height = 300, width = 800
    ) %>%
    layout(
      autosize = FALSE, yaxis = list(ticksuffix = "%"),
