...
```{r}
# For each sample and Param, a separate ANOVA is needed

anova_dt <- main_data %>%
  mutate(logVal = log(Value)) %>%
  group_split(Sample, Param)

sample_anova_dt <- anova_dt[[1]]
datatable(sample_anova_dt)
```

```{r}
# Build the linear model

model  <- lm(logVal ~ Mach, data = sample_anova_dt)
summary(model)
```
...
```{r}
ggqqplot(sample_anova_dt, "Value", facet.by = "Mach")
```
