...
pct_diff_values %>%
  ggline(
    ...
    facet.by = "Mach", ncol = 1, x.text.angle = 30, palette = pal_npg()(9)
  )
```
...
```{r}
# Build a linear model for how PCT_diff depends on Machine selection

one_way_model <- pct_diff_values %>%
  mutate(
    Mach = factor(Mach),
    Param = factor(Param)
  ) %>%
  lm(pct_diff ~ Mach, data = .)
...
pct_diff_values %>%
  ggqqplot("pct_diff", facet.by = "Mach") %>%
...
pct_diff_values %>%
  shapiro_test(pct_diff)
```
Distribution of percentage values is not normal, so non-parametric (Kruskal - Wallis)
tests are needed.
...
pct_diff_values %>%
  kruskal_effsize(pct_diff ~ Mach)
```

```{r message = FALSE, warning = FALSE, fig.width = 8, fig.height = 12}
# Compute Dunn's post hoc test
